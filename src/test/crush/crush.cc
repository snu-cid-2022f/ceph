...
void test_uniform2_reweight(int alg, int n) {
  ...
  int root0;
  crush_bucket *b0 = crush_make_bucket(c->get_crush_map(),
				       alg, CRUSH_HASH_RJENKINS1,
				       ROOT_TYPE, n - 1, items, weights);
  ...
  int sum[n];
  double totalweight = 0;
  vector<unsigned> reweightx(n - 1);
  vector<unsigned> reweight(n);
  for (int i=0; i<n; ++i) {
    sum[i] = 0;
    if (i < n - 1) reweightx[i] = 0x10000;
    reweight[i] = 0x10000;
    totalweight += weights[i];
  }
  totalweight /= (double)0x10000;
  ...
  for (int i=0; i<total; ++i) {
    vector<int> out0, out1;
    c->do_rule(rule0, i, out0, 1, reweightx, 0);
    ASSERT_EQ(1u, out0.size());
  ...
  double same_ratio = (double) same / (double) total;

  cout << "same ratio:" << (same_ratio * 100.) << std::endl;
...
#define TEST_UNIFORM2(i) TEST_F(CRUSHTest, uniform_reweight_##i) { test_uniform2_reweight(CRUSH_BUCKET_UNIFORM, i); } TEST_F(CRUSHTest, uniform2_reweight_##i) { test_uniform2_reweight(CRUSH_BUCKET_UNIFORM2, i); }
